...
export const RULES: RulesDoc[] = [
  ...
  },
  {
    key: 'soccer',
    version: 'v1',
    title: 'Capelli Sport Cup — Soccer (1-vs-1)',
    subtitle: 'Official Rules · 2 halves × 2 minutes · mBot2 · max 40 RPM · 22 × 22 cm · 1.5 kg',
    appliesTo: /capelli\s*soccer/i,
    htmlUrl: '/rules/soccer_rules.html',
    docxUrl: '/rules/soccer_rules.docx',
    sections: [],
    scoring: [
      { label: 'Each goal scored on opponent', points: '+1' },
      { label: 'Bounce-in goal (off wall or robot)', points: '+1 (counts)' },
      { label: 'Own goal', points: '+1 to opponent' },
      { label: 'Push-in (robot pushed into goal with ball)', points: 'no goal · free kick to victim' },
      { label: 'Pinning >5 s after referee count to 5', points: 'free kick to pinned team' },
      { label: 'Stalling on the ball >10 s', points: 'free kick to opposing team' },
      { label: 'Goal-mouth blocking >10 s', points: 'free kick to other team' },
      { label: 'Penalty shootout (tie-breaker)', points: '3 kicks each, then sudden death' },
    ],
    voidConditions: [
      'Robot exceeds 40 RPM speed cap or hidden boost mode → DISQUALIFICATION',
      'Forbidden attachment (sharp edges, magnets, sticky surfaces, ball-trapping mechanism) → DISQUALIFICATION',
      'Battery swap mid-match',
    ],
  },
  {
    key: 'lockerroom',
    version: 'v1',
    title: 'Capelli Starter — Locker Room Mission',
    subtitle: 'Official Rules · Ages 13–15 · Match: 150 seconds · Fully autonomous',
    appliesTo: /capelli\s*starter/i,
    htmlUrl: '/rules/lockerroom_rules.html',
    docxUrl: '/rules/lockerroom_rules.docx',
    sections: [],
    scoring: [
      { label: 'Step 1 — Leave the Locker Room', points: '+5' },
      { label: 'Step 2 — Reach the Scan Station', points: '+10' },
      { label: 'Step 3 — Coach Approval (Team Box removed)', points: '+15' },
      { label: 'Step 5 — Correct color cube delivered to big circle', points: '+20' },
      { label: 'Step 5 — Wrong color circle', points: '−10' },
      { label: 'Step 6 — Stadium Gate bonus (only if Step 5 correct)', points: '+5' },
      { label: 'Step 7 — Each token fully inside matching color target', points: '+10' },
      { label: 'Step 7 — Each token partially inside target', points: '+5' },
      { label: 'Step 7 — Each token in wrong target', points: '−10' },
      { label: 'Robot touched the Stadium Gate', points: '−10' },
    ],
    voidConditions: [
      'Anyone except the referee touches the robot or field after GO → SCORE = 0',
      'Student uses a remote, joystick, or any manual control → SCORE = 0',
    ],
  },
  {
    key: 'signalrise',
